...
Web запускается без настроенных моделей и Confluence. Worker также может собрать зависимости без этих необязательных интеграций, но не запускается без `DOCGEN_WORKER_ID`. Запуск сборки или проверки возвращает `503`, пока не настроены базовый URL и имя текстовой модели. Базовый URL и имя мультимодальной модели требуются только для проектов с изображениями или источниками Confluence. Для проекта с источником Confluence операция дополнительно возвращает `503`, пока не настроены API и токен Confluence. Задание не ставится в очередь при такой проверке зависимостей.
