...
3. Довести media из DOCX, PDF и Confluence до редактора и экспорта без обхода
   project isolation.
4. Усилить зафиксированный trusted single-user контур и явно запретить
   публичное/multi-user развёртывание до отдельного auth/ACL эпика.
...
| DG-205 | P1 | Hardening trusted single-user контура | Deployment security | Security acceptance |
...
flowchart LR
    ...
    D --> G
    I[DG-202<br/>Grounding] --> J[DG-203<br/>Injection resilience]
    K[DG-204<br/>Structured output] --> J
    J --> G
    M[DG-205<br/>Trusted contour hardening] --> G
```
...
**Действие агента:** измерить обычные и патологические fixtures, зафиксировать в PR один
точный набор дефолтов и единую семантику подсчёта пробелов. Внешнее решение для старта
не требуется.
...
**Фиксированный контракт:** таблица `media_assets` хранит UUID `id`, `project_id`,
`source_id`, внутренний `relative_path`, `media_type`, `size_bytes`, SHA-256, width,
height и `created_at`. Единый media service получает `project_id`/`source_id`;
`DocumentNode.data.media_ref` хранит только UUID `media_assets.id`; resolver проверяет
project/source ownership. MIME определяется по содержимому; действуют лимиты
bytes/pixels/count/total; source/project deletion и rollback очищают media без orphan files.
...
**Фиксированная модель provenance:** `source_id` хранит реальный `Source.id`, новое
обязательное поле `block_id` — `NormalizedBlock.id`. Legacy JSON, где `source_id`
содержит block ID, мигрируется при чтении: значение переносится в `block_id`, а Source ID
восстанавливается из префикса нормализованного block ID.

**Фиксированный режим полноты:** `lossless` для `faq`, `use-case`, `technical-spec`,
`api-docs` и `release-notes`. Каждая содержательная единица должна быть покрыта;
...
### DG-205 · Усиление trusted single-user контура

**Фиксированная deployment model:** trusted single-user service во внутреннем контуре.
Публичное и multi-user развёртывание запрещено до отдельного auth/ownership/ACL эпика.

- Локальный compose публикует только `127.0.0.1:8000:8000`; Dokploy сохраняет `expose`.
- Если принимается proxy identity, агент задаёт явный header и trusted proxy IP/CIDR;
  direct request и missing/duplicate header отклоняются.
- Текущая Confluence service account получает least privilege и allowlist по canonical
  space/page ID; каждый redirect проверяется повторно.
- Глобальные потолки projects, storage bytes и queued/running jobs проверяются атомарно.
- README и deployment config явно запрещают публикацию без trusted reverse proxy,
  выполняющего authentication.

Встроенные auth, ownership, ACL, delegated Confluence access, tenant quotas и rate limiting
не маскируются частичным решением: это отдельный multi-user эпик.

## Зафиксированные решения для реализации

| Область | Решение | Итоговая проверка |
| --- | --- | --- |
| Provenance | Раздельные `source_id` + обязательный `block_id`; backward-compatible migration | AI Architect |
| Полнота | `lossless` для пяти смысловых шаблонов | Product + AI Architect |
| Media reference | UUID `media_assets.id`, разрешаемый только через project-scoped service | Architect + Security |
| Deployment model | Trusted single-user internal; public/multi-user запрещён | Product + Security |
| Confluence credentials | Least-privilege service account + canonical allowlist | Security + Product |

Агент реализует эти решения в Этапе 2. Ревьюеры оценивают готовые код, тесты,
миграции и deployment assumptions; их виза является финальным шлюзом приёмки,
а не предварительным разрешением начать работу.
...
| Дата | Изменение |
| --- | --- |
| 2026-09-03 | P1 превращён в исполнимое задание для агента: решения media, provenance, lossless и deployment зафиксированы; architecture/security review перенесён в финальную приёмку. |
